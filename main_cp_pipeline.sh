...
# main_cp_pipeline.sh
# Usage: sbatch main_cp_pipeline.sh SAMPLE_NAME

SAMPLE=$1
THREADS=16
REF_SEED="malva.fasta"  # The fasta seed
REF_GB="malva.gb"               # The GenBank ref for standardization
...
echo "Checking if multiple scaffolds/assemblies for extension and/or standardization"
shopt -s nullglob
...
if [[ ${#COMPLETE_PATHS[@]} -ge 1 ]]; then
    ...
    if [[ -f "$GO_SCAFFOLD" ]]; then
        ...
        # Run NOVOPlasty script (which now calls standardization internally)
        # Note: We pass standard_output as $FINAL_RAW_FASTA
...
# Safety check: Did we get a standardized file?
...
echo "Getting stats from assembly"
